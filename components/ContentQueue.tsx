...
// Helper to determine source icons, moved out of the main component scope
...
// DraftCard component moved outside to solve TS key prop error and improve render performance
const DraftCard: React.FC<DraftCardProps> = ({ post, selectedIds, setSelectedIds, onApprove, onReject, onEdit }) => (
...
// ListItem component moved outside to solve TS key prop error and improve render performance
const ListItem: React.FC<ListItemProps> = ({ post, activeTab, onEdit, onPublish }) => (
  <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-4 hover:border-slate-300 transition-all shadow-sm">
    ...
    <div className="flex items-center gap-1.5 shrink-0">
      <button className="p-1.5 text-slate-400 hover:bg-slate-50 rounded-lg" onClick={() => onEdit(post.id)}><Edit3 className="w-4 h-4" /></button>
      {activeTab === 'scheduled' && (
        <button 
